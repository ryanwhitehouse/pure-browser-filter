...
export class Filter implements IFilter {
  ...
  private _handleBackgroundErrors (request: PredictionRequest, resolve: (value: PredictionResponse) => void, message: string | undefined): void {
    ...
    if (reconnectCount > 3) {
      resolve(new PredictionResponse(true, request.url, 'Background worker doesn\'t working'))
      console.warn(`[NSFW-Filter] Background worker is down, hiding ${request.url}`)
      this.requestQueue.delete(request.url)
    } else {
      request.reconectTimer = window.setTimeout(() => this._requestToAnalyzeImage(request, resolve), 150)
    }
  }
}
